...
void PUSService5::Exec5_5TC(CDTCHandler &tcHandler, CDTMList &tmList) {
	...
	offset = GetRIDEnableConfigOffset(RID);
	if (IsIndexValid(index)) {
		// Esto es para settear el bit del array que permite que ocurra el RID
		// Asi lo que hago es poner el bit a 1.

		// No hay sentencia de condicional. Hago una OR para poner ese bit a 1 si o si
...
void PUSService5::Exec5_6TC(CDTCHandler &tcHandler, CDTMList &tmList) {
	...
	index = GetRIDEnableConfigIndex(RID);
	offset = GetRIDEnableConfigOffset(RID);
	// Este tiene que funcionar de la forma contraria, tiene que deshabilitarmelo

	// En este caso tengo que dejar todos los valores iguales excepto por el bit del offset que lo que hago es ponerlo a 0, desactivarlo
	if (IsIndexValid(index)) {
